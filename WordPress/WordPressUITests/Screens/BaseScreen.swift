...
class BaseScreen {
    ...
    @discardableResult
    public func waitForElementToExist(element: XCUIElement, timeout: TimeInterval? = nil) -> Bool {
        return waitFor(element: element, predicate: "exists == true")
    }
    ...
    func tapStatusBarToScrollToTop() {
        ...
        XCUIApplication().windows.allElementsBoundByIndex.forEach {
            guard $0.isFullyVisibleOnScreen else { return }
//            guard $0.frame

            $0.coordinate(withNormalizedOffset: CGVector(dx: 10, dy: 10)).tap()
        }
    }
